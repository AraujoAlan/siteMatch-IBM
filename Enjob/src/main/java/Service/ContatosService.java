...
public class ContatosService {
    ...
    public static Response InserirContatos(Contatos co, @Context UriInfo uriInfo) throws SQLException {
        ...
        query = String.format("insert into TB_CONTATO(ID_CONTATO, NR_DDI, NR_DDD, NR_TELEFONE) values (%s,'%s',%s,%s)",
                id, co.getIdContato(), co.getDdi(), co.getDdd(), co.getTelefone());
        Dao.InsertUpdateDeleteCommand(query);

        UriBuilder builder = uriInfo.getAbsolutePathBuilder();
        builder.path(Integer.toString(co.getIdContato()));
    ...
    @Path("/{id}")
    @Consumes(MediaType.APPLICATION_JSON)
    public static void AtualizarContatos(@PathParam("id") Contatos co) {
        String query = String.format("update ID_CONTATO set contatos = '%s', ddi = '%s', ddd = %s, telefone = %s",
                co.getIdContato(), co.getDdi(), co.getDdd(), co.getTelefone());
        Dao.InsertUpdateDeleteCommand(query);
    }
